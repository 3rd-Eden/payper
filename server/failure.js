...
module.exports = async function failure({ bundle, error }) {
  //
  // Clean up the error message by removing any possible path to prevent leaking
  // somewhat private information to our users.
  //
  const message = error.replace(/\/[^\s'"]+/g, '(private)');
